...
def test_fill_state_msg_preserves_values_including_nan(packet):
    ...
    assert msg.seq == 1234
    assert msg.act_error[P.NUM_JOINTS - 1] == 0x08000200
...
def test_residual_over_the_board_limit_is_reported_not_clipped():
    ...
    assert turns[P.JOINT_INDEX["right_knee_pitch"]] == pytest.approx(-0.7 / (2 * math.pi))
    assert convert.names_of(over) == ["right_knee_pitch"]


@pytest.mark.parametrize("bad", [[0.0] * (P.NUM_JOINTS - 1), [0.0] * (P.NUM_JOINTS + 1),
                                 [float("nan")] + [0.0] * (P.NUM_JOINTS - 1),
                                 [0.0] * (P.NUM_JOINTS - 1) + [float("inf")]])
...
def test_policy_block_matches_the_raw_bytes(packet, packet_bytes):
    ...
    assert from_packet.dtype == np.float32 and from_packet.size == P.POLICY_COUNT
...
def test_joint_state_arrays(packet):
    ...
    assert names == list(P.JOINT_NAMES) + list(P.BOLTED_JOINT_NAMES)
    assert all(type(x) is float for x in pos + vel + eff)
    assert pos[4] == pytest.approx(0.5) and eff[P.NUM_JOINTS - 1] == pytest.approx(3.5)
    # The bolted waist is reported, at zero, so robot_state_publisher can place
    # everything above it.
    assert pos[P.NUM_JOINTS:] == [0.0] * len(P.BOLTED_JOINT_NAMES)
